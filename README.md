...
This advanced state machine drives the `Cache_Bonus.v` module. In addition to standard read/write handling, it manages the write-back replacement policy. You can observe the specific states dedicated to checking for dirty bits and writing evicted blocks back to main memory (`MISS_CHECK`, `WRITE_BACK_wait`, `WRITE_BACK_done`).
